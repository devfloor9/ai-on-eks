...
## [1단계: 인프라 설정](/docs/infra/inference/inference-ready-cluster)
...
[**추론 준비 EKS 클러스터**](/docs/infra/inference/inference-ready-cluster)는 AI/ML 추론 워크로드를 위해 특별히 설계된 사전 구성된 인프라를 제공합니다.
...
인프라를 프로비저닝하려면 [추론 준비 클러스터 배포 가이드](/docs/infra/inference/inference-ready-cluster)를 따르세요.

---

## [2단계: 모델 배포](/docs/blueprints/inference/inference-charts)
...
[**AI on EKS 추론 차트**](/docs/blueprints/inference/inference-charts)는 인기 모델을 위한 사전 구성된 값이 포함된 Helm 기반 배포를 제공하며, 여러 배포 프레임워크를 지원합니다.
...
상세한 구성 옵션과 고급 배포 시나리오는 [추론 차트 문서](/docs/blueprints/inference/inference-charts)를 참조하세요.

---

## [3단계: 최적화](/docs/guidance/)
...
[**가이던스 섹션**](/docs/guidance/)은 프로덕션 AI/ML 워크로드를 위한 포괄적인 모범 사례와 최적화 기법을 제공합니다. 각 기법은 특정 성능 또는 비용 과제를 해결합니다:

#### [컨테이너 시작 시간 최적화](/docs/guidance/container-startup-time/)

모델 로딩 시간을 수 분에서 수 초로 줄여 오토스케일링 응답성과 개발 반복 속도를 개선합니다.

**주요 기법**:
- **[이미지 크기 줄이기](/docs/guidance/container-startup-time/reduce-container-image-size/)**: 풀 시간을 최소화하기 위해 컨테이너 이미지 최적화
- **[모델 아티팩트 분리](/docs/guidance/container-startup-time/reduce-container-image-size/decoupling-model-artifacts)**: 컨테이너 이미지에서 모델 가중치 분리
- **[풀 프로세스 가속화](/docs/guidance/container-startup-time/accelerate-pull-process/)**: containerd 스냅샷터 및 이미지 프리패칭 사용
- **[노드에 프리패치](/docs/guidance/container-startup-time/accelerate-pull-process/prefecthing-images-on-br)**: 워크로드 스케줄링 전에 모델 이미지로 노드 예열
...
#### [동적 리소스 할당 (DRA)](/docs/guidance/dynamic-resource-allocation)
...
#### [관측성 및 모니터링](/docs/guidance/observability)
...
#### [네트워킹 최적화](/docs/guidance/networking)
...
#### [EKS 모범 사례](/docs/guidance/eks-best-practices)
...
각 최적화 기법에 대해 자세히 알아보려면 [가이던스 문서](/docs/guidance/)를 참조하세요.
...
- **EKS가 처음이신가요?** [인프라 설정](/docs/infra/inference/inference-ready-cluster)부터 시작하세요
- **인프라가 있으신가요?** [모델 배포](/docs/blueprints/inference/inference-charts)로 바로 이동하세요
- **모델이 실행 중인가요?** [가이던스](/docs/guidance/)로 최적화하세요
